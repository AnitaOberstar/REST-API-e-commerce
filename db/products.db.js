...
const createProductDb = async ({ name, price, description, category, image_url, status }) => {
  const text = `INSERT INTO products(name, price, description, image_url, status)
...
const modifyProductDb = async ({ id, name, price, description, category, image_url, status }) => {
  const text = `UPDATE products SET name=$2, price=$3, description=$4, image_url=$5, status=$6
    WHERE id = $1 RETURNING *`
  const values = [id, name, price, description, image_url, status]
